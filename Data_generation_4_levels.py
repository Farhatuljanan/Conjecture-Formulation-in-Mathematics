...
def generate_random_graph():   
 level1_nodes = [f'A{i}' for i in range(1, random.choice([2,3,4,5,6]))]
 level2_nodes = [f'B{i}' for i in range(1, random.choice([5,7,9,11,13]))]
 level3_nodes = [f'C{i}' for i in range(1, random.choice([5,7,9,11,13]))]
...
# Generate all possible edges from level 1 to level 2 and level 2 to level 3
 ...
 edges_level4_to_sink= list(itertools.product(level4_nodes, 'T'))
 ...
 G.add_edges_from(edges_level4_to_sink)
# Capacity for first level
 ...
 capacity_l2 = [2.5, 1.67, 1.25, 1, 0.83]
 if num_nodes_l2 == 4:
